...
#ifndef KZ_ANDROID_READ_ONLY_DISK_FILE_HPP
#define KZ_ANDROID_READ_ONLY_DISK_FILE_HPP
...
class ReadOnlyDiskFile : public File
{
public:
    /// Factory function.
    ...
    /// \param offset Number of bytes to skip. Must be non-negative.
    virtual void seek(SeekPosition position, int64_t offset) KZ_OVERRIDE;

    /// \note Some memory mapping operations on Android are limited to 32 bit.
    virtual const byte* map() KZ_OVERRIDE;

protected:
    /// Constructor.
    /// \param resourceFile A pointer to an open resource file.
    explicit ReadOnlyDiskFile(KzsResourceFile* resourceFile);
