...
#define KZC_INPUT_STREAM_BUFFER_DEFAULT_SIZE 4096
...
KANZI_API kzsException kzcInputStreamCreateFromFile(const struct KzcMemoryManager* memoryManager, kzString filePath,
...
KANZI_API kzsException kzcInputStreamCreateFromResource(const struct KzcMemoryManager* memoryManager, kzString resourcePath,
                                              enum KzcIOStreamEndianness endianness, struct KzcInputStream** out_inputStream);

/** Creates an input stream pointing to a given memory buffer of given size. */
KANZI_API kzsException kzcInputStreamCreateFromMemory(const struct KzcMemoryManager* memoryManager, const kanzi::byte* buffer, kzUint size,
                                                      enum KzcIOStreamEndianness endianness, struct KzcInputStream** out_inputStream);
/**
* Creates an input stream pointing to a given memory buffer of given size.
* Even if this is a stream from memory buffer, it is considered temporary and cannot be mapped.
*/
KANZI_API kzsException kzcInputStreamCreateFromMemoryTemporary(const struct KzcMemoryManager* memoryManager, const kanzi::byte* buffer, kzUint size,
                                                               enum KzcIOStreamEndianness endianness, struct KzcInputStream** out_inputStream);
 ...
 */
KANZI_API kzsException kzcInputStreamCreateFromStream(const struct KzcMemoryManager* memoryManager, struct KzcInputStream* targetInputStream, kzInt targetLength,
                                            enum KzcIOStreamEndianness endianness, struct KzcInputStream** out_inputStream);

/** Creates an input stream using custom read functions. */
KANZI_API kzsException kzcInputStreamCreateCustom(const struct KzcMemoryManager* memoryManager, void* customData,
 ...
 * \param byteCount The number of bytes that the stream will be advanced by if mapping is successful.
 * \param out_data Will be set to the memory-address of current position if mapping is successful, otherwise will be set to KZ_NULL.
 * \return KZC_EXCEPTION_END_OF_STREAM if byteCount larger that bytes left in the stream, otherwise KZS_SUCCESS.
 */
...
KANZI_API kzsException kzcInputStreamReadAllBytes(struct KzcInputStream* inputStream, const struct KzcMemoryManager* memoryManager,
                                                  kzUint* out_size, kanzi::byte** out_data);

/** Reads the rest of the input stream to a string. The caller must free the returned string using kzcStringDelete(). */
KANZI_API kzsException kzcInputStreamReadText(struct KzcInputStream* inputStream, const struct KzcMemoryManager* memoryManager,
 ...
 */
KANZI_API kzsException kzcInputStreamReadTextLines(struct KzcInputStream* inputStream, const struct KzcMemoryManager* memoryManager,
